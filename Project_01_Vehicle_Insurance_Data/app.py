...
# Allow all CORS origins for simplicity (you can restrict this in production)
origins = ["*"]

# Configure middleware to handle CORS (Cross-Origin Resource Sharing)
app.add_middleware(
    CORSMiddleware,
...
class DataForm:
    """A form class for handling vehicle data input."""
...
async def trainRouteClient():
    ...
    try:
        ...
        return Response("Training completed successfully", media_type="text/plain")
    except Exception as e:
        return Response(f"An error occurred during training: {str(e)}", media_type="text/plain")
...
async def predictRouteClient(request: Request):
    """
    Endpoint to handle form submission and make predictions.
    """
    try:
        ...
        # Make prediction using the model and get the result
        value = model_predictor.predict(dataframe=vehicle_df)[0]

        # Interpret the prediction result and prepare the response message
        status = "Response-Yes" if value == 1 else "Response-No"

        # Render the result on the same page with the prediction status
        return templates.TemplateResponse(
            "vehicledata.html", 
            {
                "request": request, 
                "context": f"Prediction Result: {status}"
            }
        )
    except Exception as e:
        return Response(f"An error occurred during prediction: {str(e)}", media_type="text/plain")
